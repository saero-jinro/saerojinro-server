...
public interface FileAdminController {
	@Operation(
		summary = "강의 썸네일 파일 업로드",
		description = "강의 썸네일 업로드",
		responses = {
			@ApiResponse(
				responseCode = "201",
	...
	@Operation(
		summary = "강연자 프로필 사진 파일 업로드",
		description = "강연자 사진 업로드",
		responses = {
			@ApiResponse(
				responseCode = "201",
				content = @Content(schema = @Schema(implementation = FileSaveResponse.class))
	...
	@Operation(
		summary = "강의 자료 파일 업로드",
		description = "강의 자료 업로드",
		responses = {
			@ApiResponse(
				responseCode = "201",
				description = "파일 업로드 성공",
				content = @Content(
					schema = @Schema(implementation = FileSaveResponse.class)
				)
			)
		}
	)
	ResponseEntity<FileSaveResponse> uploadLectureMaterials(
		@Parameter(
